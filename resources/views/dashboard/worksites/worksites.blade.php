...
@section('title', __('content.list',['model'=>trans_choice('content.work site',2)]))
...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ trans_choice('content.work site',2) }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('content.list',['model'=>trans_choice('content.work site',2)]) }}</span>
						</div>
					</div>
					<div class="d-flex my-xl-auto right-content">
						@can(__('content.work-create'))
							<div class="pr-1 mb-3 mb-xl-0">
								<a href="{{ route('worksites.create') }}" class="btn btn-success ml-2"><i class="mdi mdi-plus ml-1"></i>{{ __('sidebar.add new') }}</a>
							</div>
						@endcan
										...
										<div class="dropdown-menu dropdown-menu-left" aria-labelledby="dropdownMenuDate" data-x-placement="bottom-end">
											@isset($years)
												@forelse ($years as $item)
													<a class="dropdown-item" href="{{ route('worksites.index',['year'=>$item->year]) }}">{{ $item->year }}</a>
												@empty
													<a class="dropdown-item" href="#">{{ __('content.no items yet',['model'=>trans_choice('content.work site',2)]) }}</a>	
												@endforelse
											@endisset
										</div>
									@endif
					...
					<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 grid-margin">
						<div class="card">
								...
								<div class="d-flex justify-content-between">
									<h4 class="card-title mb-2">
										@if (isset(request()->search))
											@if ($worksites->total() > 0)
												<b>{!! __('content.there are (n) results found for (s)', ['number'=>$worksites->total(),'search'=> "(".request()->search.")"]) !!}</b>
											@else
												<b>{{ __('content.sorry, no results found') }}</b>
											@endif
										@else
											{{ __('content.table',['model'=>trans_choice('content.work site',2)]) }}
										@endif
									</h4>
							...
							<div class="card-body">
									...
									<table class="table card-table table-striped table-vcenter text-nowrap mb-0">
										<thead>
											<tr>
												...
												<th class="wd-lg-20p"><span>{{ __('content.name',['model'=>trans_choice('content.work site',1)]) }}</span></th>
												<th class="wd-lg-20p"><span>{{ __('content.name',['model'=>__('content.owner')]) }}</span></th>
										...
										<tbody>
											@forelse ($worksites as $worksite)
												<tr>
													...
													<td>{{ $worksite->name }}</td>
													<td>{{ $worksite->owner }}</td>
													<td>{{ $worksite->user->name }}</td>
													<td>{{ $worksite->created_at }}</td>
													<td>
														@can(__('content.work-update'))
															<a href="{{ route('worksites.edit', $worksite->id) }}" class="btn btn-sm btn-info"><i class="las la-pen"></i></a>
														@else
															<a href="javascript:;" class="btn btn-sm btn-secondary disabled"><i class="las la-pen"></i></a>
														@endcan
														...
														@can(__('content.work-delete'))
															...
															<form action="{{ route('worksites.destroy',$worksite->id) }}" method="post" class="d-none">
																@csrf
																@method('DELETE')
															</form>
														@else
								...
								{!! $worksites->appends( request()->query() )->links('dashboard.paginations.pagination') !!}
							</div>
						</div>
					</div><!-- COL END -->
				</div>
