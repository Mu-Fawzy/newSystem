...
@section('title', __('content.edit')." - ".$worksite->name)
@section('css')
...
@section('page-header')
		...
		<div class="breadcrumb-header justify-content-between">
			<div class="my-auto">
				<div class="d-flex">
					<h4 class="content-title mb-0 my-auto">{{ trans_choice('content.work site',2) }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('content.edit')." - ".$worksite->name }}</span>
				</div>
			</div>
					...
					<div class="card">
						<div class="card-header pb-0">
							<div class="d-flex justify-content-between mg-b-20">
								<div>
									<h5 class="main-profile-name">{{ __('content.edit')." - ".$worksite->name }}</h5>
								</div>
						...
						<div class="card-body">
							<form class="form-horizontal" action="{{ route('worksites.update', $worksite->id) }}" method="Post">
								@csrf
								@method('PUT')
									...
									@foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
										<div class="col-lg-6">
											<div class="form-group">
												...
												<input type="text" class="form-control" name="name_{{ $localeCode }}" placeholder="{{  __('content.name',['model'=>trans_choice('content.work site',1)])." ".$localeCode }}" value="{{ $worksite->getTranslation('name', $localeCode) }}">
											...
											<div class="form-group">
												...
												<input type="text" class="form-control" name="owner_{{ $localeCode }}" placeholder="{{ __('content.name',['model'=>trans_choice('content.owner',1)])." ".$localeCode }}" value="{{ $worksite->getTranslation('owner', $localeCode) }}">
